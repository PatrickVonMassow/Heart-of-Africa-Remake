...
- Controls suitable for mouse/keyboard and gamepad. The use key is **Space**: it addresses the village elder, enters the functional building at whose door the traveller stands (§2.3), and confirms entering a settlement from the bird's-eye view. The chronicle/journal is opened and closed with the Tab key (gamepad: Y). Holding Ctrl names the animals, people and usable objects on screen (§17.8). Tab's default focus cycling is suppressed while playing so it does not shift focus onto UI controls; inside form controls (debug-menu fields, dialog inputs) Tab still navigates between them normally.
...
- **Holding Ctrl changes nothing else.** The world runs on, the character neither stops nor steers, and the key combinations the browser owns are left to it.
