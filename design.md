...
- Picker: graphics quality level — low / medium / high (§21.1 F9), default medium; also cycled with F9. This is the ONLY graphics control in the menu — selecting a level drives every render lever through effective selectors. The individual per-setting graphics allow-flags (TRAA, SSAO, half/full shadows, campfire shadows) are NO LONGER exposed in the menu — they remain internal store fields, set by the mobile touch-quality preset (§17.5) and the F8 benchmark and combined by the effective selectors, so the menu stays uncluttered while the level still fully determines the look.
