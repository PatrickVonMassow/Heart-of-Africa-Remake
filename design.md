...
- A **"Low Details" performance mode** (toggled with F7 / a debug-menu checkbox, §21) can trade this chain down for framerate on weak hardware. It is off by default and picture-neutral until enabled — the full pipeline above is the shipped look. When on it caps the render resolution and drops the post chain and shadows first (the fill-rate levers the real-hardware benchmark proved dominant, §21.1 F8), then thins geometry (terrain refinement, flora) as a secondary lever for genuinely geometry-bound GPUs. It reads through effective render selectors that never overwrite the player's individual graphics settings, so it is fully reversible. See §21.
...
Map-point labels are gated by discovery: the floating name of a settlement or a natural landmark (village, waterfall, mountain, lake, river) appears only once the traveller has discovered it — a place once it has been visited, a landmark once it has been sighted (the same sighting that earns its discovery bounty, §10). Until then the point carries a muted "?" as its label instead of the name. The exploration overview likewise draws only the places already visited. The bird's-eye "Space to enter <name>" hint (§2.3) reads the SAME discovery gate: it shows "?" for an undiscovered settlement rather than leaking its name.
...
First-person view (settlements): walkable space, use-key prompts at buildings/persons ("Space — <name>", shown once the traveller stands at the door or by the elder), trade and dialog windows. In the bird's-eye view the settlement's own prompt reads "Space to enter <name>" while the traveller is within its enter radius (§2.3) — with "?" in place of the name while the settlement is still undiscovered (§17.2). A gift to a native additionally provides an orientation over the settlement's buildings, with the important, enterable buildings highlighted.
...
- Entering a settlement puts the focus straight on the controls: any lingering HUD button is blurred so the keyboard controls the character at once, and mouse-look (pointer lock) engages straight away — the Space use-key press that entered carries the user activation the browser requires. A modal dialog releases the lock so its buttons stay clickable (Escape releases it too); where a browser refuses the un-clicked request, a deliberate click on the view remains as the fallback. Mouse-look is never grabbed while a full-screen overlay is up (the initial checkpoint-load choice, defeat or victory): the cursor is needed to click it.
...
- A blood stain is not an object lying on the ground but a property OF the ground: the round patch of earth under a kill or a trample is itself tinted red (dark, soaked, glistening), so it follows every fold of the relief exactly. Nothing floats above the terrain and nothing pokes through the pool — neither the half-moon a horizontal disc got sliced into on a hillside, nor the see-through hole a slope-tilted disc still left where the ground rose through its middle.
...
- The drama reuses §19.8's grammar without a second copy: the seized victim struggles through the SAME few-seconds window at the waterline, a charging parent that arrives in time is taken in its place (the calf escapes), one that only got close is taken alongside it, and the parent's attack rolls the same legible defence — a strong parent can drive the crocodile off its victim, but NOTHING kills the armoured ambusher (structurally, like the lion). A driven-off crocodile genuinely WITHDRAWS: it keeps to its water for a calibratable rest before it may ambush again, so the victim it just released — still standing at the bank, and a legal target again under the broadened waterline trigger — is not seized back the next moment and the parent's defence reads as the rescue it was. While it grips and consumes, the seized victim lies at the crocodile's JAWS (its mouth end, not resting on its back) and the crocodile animates as FEEDING — the classic death-roll head thrash paired with a gulp bob — so the meal reads as eating. A crocodile kill leaves no carcass on the bank: it drags the body under, and the river keeps it — no vulture ever lands for it.
...
**The campfire casts shadows (point 289, opt-in).** The fire pit's flickering point light used to illuminate straight through whatever stood between it and the ground — the player, the ring stones, a kneeling cook — which read implausible (user report). With the debug menu's "Campfire shadows" toggle the fire light renders a low-resolution cube shadow map, so stones, logs, figures and the player's own body (an invisible occluder proxy that follows the first-person camera) block the firelight and lay radial shadows on the ground; the global shadow switch gates it too. The toggle ships OFF, so the default picture is unchanged: the measured cost is about +1.5 ms per frame on the headless rig — six extra cube-face render passes; the map resolution itself is nearly free — and the verdict on whether that is affordable belongs to the user's real hardware, judged with the FPS counter in a village (the F8 benchmark routes only the bird's-eye states). If it proves cheap there, turning it on by default is a one-flag change.
...
- Checkbox: "Low Details" performance mode (§2.7/§21.1), off by default; also toggled with F7. On, it trades visible quality for framerate by driving the render levers down (resolution/dpr, post, shadows, then terrain and flora) through effective selectors, without overwriting the individual graphics settings — so turning it off restores the player's exact picture.
