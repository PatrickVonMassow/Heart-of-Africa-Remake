...
- Walking while zoomed out keeps the picture consistent: the sea surface stays aligned with the land, the far sheet matches the detailed ground's tone (it bakes in the ground textures' mean response), and beyond a zoom threshold the chunk-bound dressing (trees, rocks …) hides — it only ever surrounds the traveller and would otherwise read as a dressed rectangle on the sheet; it returns as the zoom drops back. The dressing streams zoom-aware, like the wildlife: every plant within a circle of the view radius plus a margin is drawn (so the streaming edge is always just BEYOND the view, never popping in sight), and the placement is rebuilt only once the traveller has moved a hysteresis step — so driving back and forth across a chunk boundary no longer makes the far plants jump in and out (point 164).
