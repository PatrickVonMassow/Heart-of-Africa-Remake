...
- **Three graphics quality levels — low / medium / high** (cycled with F7 / a debug-menu picker, §21), default **medium**, trade this chain against framerate. Every quality-relevant render setting is mapped per level in one registry (`src/config/quality.ts`, `QUALITY_PRESETS`); the render consumers read the current level's value through effective selectors that never overwrite the player's individual debug settings (those stay available to tune a feature within a level). **Low** is the frugal floor for weak GPUs (device pixel ratio capped to 1, the whole post chain off, low-resolution sun shadows, no campfire shadows, terrain refinement off, a tighter flora radius, thinned haze/rain) — leading with the fill-rate levers the real-hardware benchmark proved dominant (§21.1 F8). **Medium** is the balanced default (SSAO off, TRAA + Bloom on, native resolution, normal 2048 sun shadows, the 256² campfire shadows on, full geometry). **High** is the richest (SSAO on, sharper 4096 sun shadows, the softer/higher-res 512² campfire-shadow variant, everything else full). Because it is a registry, every FUTURE optical feature declares its low/medium/high behaviour there (and may offer a costlier high-only variant) — the completeness gate (§21) fails a preset that omits a setting. See §21.
...
- **F7** cycles the **graphics quality level** — low / medium / high (also a debug-menu picker), default **medium**. Each press steps DOWN one level and wraps the bottom back to the top: **medium → low → high → medium**. Every quality-relevant render setting is mapped per level in one registry (`src/config/quality.ts`); the render consumers read the current level's value through effective selectors that never overwrite the player's individual debug settings — those stay available to tune a feature within a level for testing. The levers each level moves are, in the order the real-hardware benchmark (§21.1 F8, point 277) proved they pay: the render resolution / device pixel ratio (capped to 1 on low), then the whole post pipeline (SSAO on only in high; TRAA + Bloom off only on low), then the shadows (sun-shadow map resolution 1024 / 2048 / 4096 across the levels, above today's default on high; campfire cube shadows off on low, the 256² variant on medium, the softer 512² variant on high); then, as secondary geometry levers for genuinely weak GPUs, the near-ring terrain refinement (off on low) and the flora streaming radius (tightened on low, with the low dressing — bush, papyrus, rock — no longer casting shadows), plus thinned haze/rain on low. The mobile touch-quality preset (§17.5) stays a subset of low. A localized toast names the new level ("Grafik: Niedrig/Mittel/Hoch" · "Graphics: Low/Medium/High"). **Convention:** every FUTURE optical feature MUST declare its low/medium/high value in the registry (and may offer a costlier high-only variant); a pure completeness gate (`src/config/quality.test.ts`) fails any preset that omits a quality key, so a new visual can never silently skip the levels.
...
- Picker: graphics quality level — low / medium / high (§2.7/§21.1), default medium; also cycled with F7. Selecting a level drives the render levers (resolution/dpr, post, shadow resolution, campfire-shadow variant, then terrain and flora) through effective selectors, without overwriting the individual debug settings — so the debug allow-flags below still tune a feature within a level.
