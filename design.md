...
- Close-range animals WALK, they do not glide: a settlement animal (the village goats) swings its legs on a gait phase driven by the distance it has travelled (stride matches speed, legs still at rest) and turns its body to FACE its direction of travel, so it never slides footless nor drifts backward while facing forward. (The bird's-eye travel fauna, seen far off where legs are imperceptible, already turns toward its heading; a close-zoom pass there would extend the same gait.)
...
- The ambush: an animal that comes to the bank to drink (the §19.7 shore visit) inside the calibratable strike radius triggers the lunge — a fast, short, VISIBLE burst out of the water, never a teleport. A missed lunge slinks back under and the wait resumes. Among the bank drinkers the crocodile strongly PREFERS a drinking calf or foal (a calibratable juvenile-at-the-drink preference, §21.2), so the §19.8 sacrifice/rescue drama plays out at the waterline the more often.
