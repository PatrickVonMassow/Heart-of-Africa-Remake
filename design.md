...
- Checkbox: temporal anti-aliasing (TRAA, §2.7), off by default while its render-pipeline rollout is being confirmed on real hardware; when off, anti-aliasing falls back to the render pass' multisampling.
...
- The sea reads as plain ocean out to the horizon of the widest view: beyond the elevation dataset's edge the water renders as uniform deep ocean (never clamped edge streaks or false shallow blocks), and the land trimmed from the map (§3.1) lies under the same deep tone as the open sea around the continent.
