...
- **F5** opens/closes the state-dump popup for bug reports: the COMPLETE game state — every field of the game store, the full balance object (so every debug override is visible), the transient UI state and a self-describing header (app/build marker, generation date) — as pretty-printed JSON in a top-most modal (§17.4). The text is read-only and selectable; a **Download** button saves it as `hoa-state-<YYYY-MM-DD>-<seed>.json`, a **Copy** button puts it on the clipboard, and Esc or F5 closes it. Opening never moves focus onto a control (§17.5). The game intercepts F5 (the browser's reload key) while it has focus, so the popup opens instead of reloading; a real reload stays available via Ctrl+R and the browser UI.
