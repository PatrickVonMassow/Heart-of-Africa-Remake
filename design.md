...
- An orphan is adopted (point 262): when a juvenile's parent DIES — by any cause below (a predator's kill or sacrifice, the crocodile, the trample, a drowning) or simply streamed away — the calf does not stay a permanent orphan. The nearest ELIGIBLE adult of its own kind within a calibratable adoption radius (§21.2) takes it in and becomes its new parent, and the calf resumes following and leashing to it. Because the family dramas all read the parent↔child link, they then RECUR for the new pairing — the new parent will shield, sacrifice, grieve and rescue the adopted young exactly as its own — so the sacrifice and grief scenes happen more often instead of a single orphaning that ends the drama. Eligibility keeps the world legible and the links sound: an adopter is a live ADULT of the young's own species (the herds are single-species, so this is the nearest suitable herd-mate), never a predator, never the hunter that just took the parent, never the calf itself, and never one already raising a calf of its own (the parent↔child relation stays one-to-one). If no eligible adult is within reach the young simply stays parentless — a re-check each moment picks it up the instant a herd-mate roams near, and until then nothing dangles or breaks (a lion cub whose lioness dies finds no non-predator adopter and so keeps to itself). One ending comes FIRST, though (point 311): a calf freed by its parent's SACRIFICE runs its escape before it may be adopted. The adoption otherwise claimed it the instant the parent fell, and the re-parented calf turned back toward its new parent — past the feeding predator — instead of fleeing, so the sacrifice never read as an escape at all. A calibratable escape window (§21.2) holds the adoption off until the young is clear; it is a hard deadline, and when it expires the adoption picks the calf up as usual, so the guarantee is deferred, never dropped.
...
- Herd family-drama values (§19.8): the parental rescue burst factor, the calf fraction per herd group, the calf leash/play/hop-bout values, the juvenile-prey and drinking-juvenile crocodile preferences, the orphan adoption radius — the reach within which the nearest eligible adult adopts a juvenile whose parent has died (point 262), so the §19.8 dramas recur for the new pairing — and the escape run of a calf freed by its parent's sacrifice (point 311), the window during which that adoption keeps off so the freed young actually flees the predator.
