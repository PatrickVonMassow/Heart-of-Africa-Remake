...
- In the bird's-eye view, reaching a settlement's enter radius shows the hint "Space to enter <name>" (localized, with the settlement's own name), and pressing the Space use key enters it and switches to the first-person view. While the hint shows, the settlement's map name-label is hidden (the hint stands in its place). Reaching the radius alone does not enter — the deliberate Space press does, so the traveller is never pulled in against his will.
...
First-person view (settlements): walkable space, use-key prompts at buildings/persons ("Space — <name>", shown once the traveller stands at the door or by the elder), trade and dialog windows. In the bird's-eye view the settlement's own prompt reads "Space to enter <name>" while the traveller is within its enter radius (§2.3). A gift to a native additionally provides an orientation over the settlement's buildings, with the important, enterable buildings highlighted.
