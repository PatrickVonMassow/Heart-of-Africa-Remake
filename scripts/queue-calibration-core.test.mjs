...
describe('distributions', () => {
  ...
  it('counts a class that has no measurable span but still exists', () => {
    const rows = [...classOf(6, 0.3), landing({ point: 9, delegated: false, elapsedHours: null })]
...
describe('is one global factor honest?', () => {
  ...
  it('REFUSES it when the classes differ, and names the axis that refused', () => {
    const rows = [
      ...
      ...classOf(6, 0.25, { criticality: 'high', picture: false, delegated: false, elapsedHours: 0.5 }),
  ...
  it('measures the spread only across classes that have comparables', () => {
    const summaries = [
      { comparable: true, ratio: { median: 0.3 } },
      { comparable: true, ratio: { median: 0.6 } },
      { comparable: false, ratio: { median: 99 } },
    ]
    expect(axisSpread(summaries)).toEqual({ classes: 2, spread: 2 })
    expect(axisSpread([summaries[0]])).toEqual({ classes: 1, spread: null })
  })
})
...
describe('the rewrite plan', () => {
  ...
  const rows = [
    ...classOf(6, 0.25, { criticality: 'medium', picture: false, delegated: true }),
    ...classOf(6, 1.2, { criticality: 'high', picture: true, delegated: false, elapsedHours: 2.4 }),
  ...
  it('uses the global factor for every card once one was adopted', () => {
    const flat = classOf(8, 0.5, { picture: false, delegated: true })
      .map((r, i) => ({ ...r, point: 600 + i, criticality: i % 2 ? 'low' : 'high', picture: i % 3 === 0, delegated: i % 4 !== 0 }))
    const uniform = calibrationReading(flat)
    if (uniform.decision.adopted) {
      expect(factorForCard(uniform, 'anything-at-all').basis).toBe('global')
    }
    // The measured repository refuses it, so the fallback is what must hold.
