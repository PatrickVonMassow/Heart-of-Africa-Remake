 ...
 * label-only card like `Closing-Aufräum + Fable`); `points` is the shared
 * parser's leading run, so a combined card may name several while numbers
 * inside the prose are ignored.
 */
export function parseCardTitle(raw, options = {}) {
  ...
  return {
    ...
    points,
    label: ownership.prefixEnd ? trimmed.slice(ownership.prefixEnd).trim() : trimmed,
  }
}
 ...
 */
export function nowCardTitles(html, options = {}) {
  ...
  for (const chunk of section.split(/<details\b/).slice(1)) {
    ...
    const chipPoints = num ? pointNumbersFromChip(num[1]) : []
    const parsed = chipPoints.length
      ? { raw: `${num[1].trim()} — ${t[1].trim()}`, point: chipPoints[0], points: chipPoints, label: t[1].trim() }
      : parseCardTitle(t[1], options)
