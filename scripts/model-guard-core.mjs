...
/** A trailer naming a model too weak to run the batch. */
export const DEGRADED = /\bhaiku\b/i
 ...
 *  field but still parse — they simply never match DEGRADED). */
...
/** Commits at/after sinceMs whose co-author trailer names a degraded model. */
export function findDegradedCommits(logText, sinceMs) {
  const hits = []
  for (const line of String(logText ?? '').split(/\r?\n/)) {
    const c = parseLogLine(line)
    if (!c || c.when < sinceMs) continue
    if (DEGRADED.test(c.trailers)) hits.push({ sha: c.sha, trailer: c.trailers.trim() })
  }
  return hits
}
