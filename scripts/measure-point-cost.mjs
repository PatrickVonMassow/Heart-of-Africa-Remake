...
export function formatMarkdown(snapshot) {
  ...
  const lines = [
    ...
    '',
    `Generated by \`node scripts/measure-point-cost.mjs --points ${snapshot.window.points}${refArg}\` from the harness records at ${snapshot.generatedAt}.`,
    '',
...
if (isMainModule(import.meta.url)) {
  ...
  const snapshot = buildSnapshot({
    ...
    source: {
      ...
      since: iso(since),
      until: iso(Math.max(...landed.map((row) => dateMs(row.landedAt)).filter(Number.isFinite))),
      ref,
    },
  })
