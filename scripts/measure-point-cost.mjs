...
export function readProviderTurns({ claudeDir, codexDir, since = 0, until = Infinity, points = [] } = {}) {
  const turns = []
  const firstTimestamp = (text) => {
    const value = /"timestamp"\s*:\s*"([^"]+)"/.exec(text)?.[1]
  ...
  for (const entry of claudeFiles) {
    ...
    const openedAt = firstTimestamp(text)
    if (openedAt != null && openedAt > until) continue
    claudeRead += 1
    const parsed = parseClaudeTranscript(text, { file: entry.rel, scope: entry.scope }).filter(
      (turn) => turn.at >= since && turn.at <= until,
    )
    turns.push(...parsed)
  }
  ...
  for (const path of codexFiles) {
    ...
    const parsed = parseCodexTranscript(text, { file: path }).filter((turn) => turn.at >= since && turn.at <= until)
    codexCandidates += 1
    if (candidateMatch) codexRead += 1
    turns.push(...parsed.map((turn) => ({ ...turn, candidateMatch })))
  }
  return { turns, claudeFiles: claudeRead, codexFiles: codexRead, codexCandidates }
}
...
export function buildSnapshot({ landed, turns, boundaryText = '', source = {}, generatedAt = new Date().toISOString() } = {}) {
  ...
  const sourceUntil = dateMs(source.until) ?? Math.max(...landed.map((row) => dateMs(row.landedAt)).filter(Number.isFinite))
  // A historical --ref snapshot must not absorb later traffic merely because a
  // long-lived transcript file was modified after the referenced window closed.
  const sourceWindowTurns = turns.filter(
    (turn) => (sourceSince == null || turn.at >= sourceSince) && (!Number.isFinite(sourceUntil) || turn.at <= sourceUntil),
  )
  const assigned = assignPoints(sourceWindowTurns, points)
  ...
  return {
    ...
    accounting: {
      ...
      basis: 'raw counters; every recorded token has weight 1',
      ...
      comparison: 'This is not the weighted proxy from measure-context-cost-core.mjs, where COST_WEIGHTS values cache reads at 0.1 and output at 5.',
    ...
    wholeDocumentReads: {
      ...
      sourceWindow: assigned.filter((turn) => turn.signals?.wholeDocumentRead).length,
    },
    assignedBy,
    ...result,
  }
...
export function formatMarkdown(snapshot) {
  ...
  const codexMatched = snapshot.source.codexFiles ?? 0
  const codexResidual = snapshot.residualTokens.openai
  const refArg = snapshot.source.ref && snapshot.source.ref !== 'main' ? ` --ref ${snapshot.source.ref}` : ''
  const untilArg = snapshot.source.until ? ` --until ${snapshot.source.until}` : ''
  const lines = [
    '# Cost per finished point',
    '',
    `Generated by \`node scripts/measure-point-cost.mjs --points ${snapshot.window.points}${refArg}${untilArg}\` from the harness records at ${snapshot.generatedAt}.`,
    '',
    `${snapshot.accounting.unit}; ${snapshot.accounting.note}`,
    '',
    `Counting basis: ${snapshot.accounting.basis}. ${snapshot.accounting.comparison}`,
    '',
...
if (isMainModule(import.meta.url)) {
  ...
  const providers = readProviderTurns({ claudeDir, codexDir, since, until, points: landed.map((row) => row.point) })
