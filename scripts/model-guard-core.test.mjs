...
import { DEGRADED, parseLogLine, findDegradedCommits } from './model-guard-core.mjs'
...
describe('parseLogLine', () => {
  ...
  it('keeps a trailer field that itself contains separators intact', () => {
...
describe('DEGRADED matching', () => {
  it('matches the real trailer of the 24.07 incident and case variants', () => {
    expect(DEGRADED.test('Claude Haiku 4.5 <noreply@anthropic.com>')).toBe(true)
    expect(DEGRADED.test('claude haiku 4.5 <noreply@anthropic.com>')).toBe(true)
    expect(DEGRADED.test('Claude Haiku 5 <noreply@anthropic.com>')).toBe(true)
  })
  it('passes every full-strength model and the empty merge-commit trailer', () => {
    for (const t of [
      ...
      '',
    ]) expect(DEGRADED.test(t)).toBe(false)
  })
})

describe('findDegradedCommits', () => {
  const log = [
  ...
  it('flags only degraded commits at/after the baseline', () => {
    const hits = findDegradedCommits(log, T0)
    expect(hits.map((h) => h.sha)).toEqual(['3333333', '5555555'])
  })
  it('the baseline boundary is inclusive', () => {
    const hits = findDegradedCommits(line('9999999', '2026-07-24T22:00:00Z', 'Claude Haiku 4.5 <x@y>'), T0)
    expect(hits).toHaveLength(1)
  })
  it('is empty on empty/absent input', () => {
    expect(findDegradedCommits('', T0)).toEqual([])
    expect(findDegradedCommits(null, T0)).toEqual([])
  })
})
