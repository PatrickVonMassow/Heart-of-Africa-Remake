...
export const VERIFICATION_LADDER = [
  ...
  '  (docs/picture-check-cost.md §7, 09.09.2026). The gap between them is why a long run stops',
  '  looking normal — the plan says 80m 48s for a both-backend LARGE and the five measured runs',
  '  took 115.3-120.9 min. The band is for recognising an OUTLIER (118 is inside it, 200 is not),',
  '  never a target: the machine is shared and a second author was on it for part of the window.',
  ...
  '  1. ONE SECTION of one suite — `npm test -- <suite> --section=<name>` (point 566). MEASURED',
  '     ON `polish` ALONE: 0.2-7.4 min, median 2.9 over 16 runs, against 9.9-61.5 (median 55.2)',
  '     for the whole `polish` pass. No other suite\'s sections were timed, so those cost an',
  '     unknown amount and this band says nothing about them. A section run runs that',
  '     block\'s setup and its checks and nothing else. The names come from the suite\'s own',
  '     source: `node scripts/verify/run-all.mjs <suite> --section=list` refuses the name and PRINTS',
  '     every real one, in a tenth of a second and without booting a browser.',
  ...
  '  first failure WHILE YOU ITERATE. Every run the GATE stands on FINISHES instead — the',
  '  point\'s own covering rung on the merge candidate, and the bundle\'s pass on `main`.',
  '  Finishing does not make a `--section` rung coverage: it stays PARTIAL either way.',
