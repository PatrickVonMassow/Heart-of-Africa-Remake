...
describe('the manifest a pass carries — the material states its own shape', () => {
  ...
  it('names the files beyond the reach of any pass, so nobody assumes they were read', () => {
    ...
    expect(text).toContain('Files no runnable pass can carry are named separately below and remain owed.')
    expect(text).not.toContain('cover the complete changed file set')
