...
describe('the command', () => {
  ...
  it('EXITS 1 WHEN A HALF CANNOT PROVE ITS AUTHOR — untracked, or tracked without a model field', () => {
    ...
    const claimed = run('--a', p('A.json'), '--b', p('tracked-b.txt'), '--model-b', 'GPT-6 Astra', '--union', p('U.json'), '--merged-by', 'Fable 5.1')
  ...
  it('refuses when a claimed list author contradicts per-message transcript metadata', () => {
    ...
    const r = run(
      '--a', p('A.json'), '--b', p('B.txt'), '--model-b', 'GPT-6 Astra',
  ...
  it('refuses a merger that contradicts the switch', () => {
    // The union's own merger (Astra) is not the one the ON switch owes (Fable).
  ...
  it('refuses a --merged-by flag that contradicts the committed union, and a union naming nobody', () => {
    // The flag used to MASK the committed union: a union naming Astra passed with
  ...
  it('judges the committed union spelling, so a family-wide flag cannot bridge two models', () => {
    // Committed "GPT-7 Astra" beside --merged-by "Astra": the flag matches both the
    // union and the expected merger by family, but the artefact's own spelling
    // is a DIFFERENT Astra than the switch owes — and that is what is judged.
    const other = write('U-gpt7.json', {
      mergedBy: 'GPT-7 Astra',
      entries: [
        { id: 'U1', from: ['A1'] },
        { id: 'U2', from: ['A2', 'B1'], defect: 'the badge overlaps the date' },
      ],
    ...
    git('add', 'U-gpt7.json')
    git('commit', '-q', '-m', 'File a union folded by a different Astra version')
    const r = runOff('--a', p('claude-a.json'), '--b', p('B.json'), '--union', other, '--merged-by', 'Astra')
    expect(r.status).toBe(1)
    expect(r.out).toMatch(/is not the one this stage owes/)
  })
  ...
  it('refuses a hand-stated outage in place of the switch-owned merger reason', () => {
    const r = run(
      ...
      'GPT-6 Astra',
      '--fallback',
      'GPT-6 Astra was unreachable in this session',
    )
    expect(r.status).toBe(1)
    expect(r.out).toMatch(/is not the one this stage owes/)
  ...
  it('a single TRACKED half still excludes its known author from the merge — and never claims "wrote neither half"', () => {
    ...
    const r = run('--a', p('claude-a.json'), '--b', p('B.txt'), '--model-b', 'GPT-6 Astra')
    expect(r.status).toBe(0)
  ...
  it('selects Astra while off, decorrelates its prompt, and records the switch fallback as weaker', () => {
    ...
    expect(prompt.out).toContain('MERGING MODEL — GPT-6 Astra')
    ...
    expect(countedOff.out).toContain('--merged-by "GPT-6 Astra"')
  })
  ...
  it('does not turn "Fable is off" into "the merger wrote a half" when no author is the merger', () => {
    ...
    // temp file naming nobody, so the merger Astra matches no author at all — and
    ...
    expect(r.status).toBe(0)
    expect(r.out).toContain('MERGING MODEL — GPT-6 Astra')
    expect(r.out).not.toContain('it wrote a half itself')
    expect(r.out).toContain('it wrote no KNOWN half')
    // And the framing is still owed, because the unnamed half could be Astra's own.
  ...
  it('keeps the committed spelling of a tracked half against a family-equivalent flag', () => {
    ...
    // "GPT-7 Astra" over a committed "Astra" would shift which exact model the
