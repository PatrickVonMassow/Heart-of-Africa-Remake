...
import { openStateStore, readJournal, readSnapshot, writeFileAtomic } from './batch-state.mjs'
...
function probeOf(identity) {
  const pid = typeof identity === 'object' ? identity?.pid : identity
  const expectedStartedAt = typeof identity === 'object' ? identity?.pidStartedAt : null
  if (!Number.isInteger(pid) || pid < 1) return { live: false }
  const probe = probePid(pid)
  // A negative signal-0 probe proves that the RECORDED process is absent, but
  // naturally cannot rediscover that dead process's start time. Carry the
  // recorded start identity into that negative verdict so the pair table can
  // bind the proof to the record it was taken for. A live probe always uses
  // the OS-observed start time, which is what detects PID reuse.
  return {
    live: probe?.exists === true,
    pid,
    startedAt: probe?.exists === true ? (probe.startedAt ?? null) : (expectedStartedAt ?? null),
  }
}
...
export function gatherEvidence({ repoDir = REPO_ROOT, batchId } = {}) {
  ...
  const pair = {
    ...daemonPairResolution({
      record,
      copy: lock?.daemon ?? null,
      probe: record ? probeOf(record) : null,
      generationOrder: daemonGenerationOrder({ entries: journal.entries, record, copy: lock?.daemon ?? null }),
    }),
...
export function applyPairResolution({ repoDir = REPO_ROOT, batchId, report, sessionId = null } = {}) {
  ...
  const writeLock = (next) => writeFileAtomic(lockPath, `${JSON.stringify(next)}\n`)
