...
360 scripts. Purpose and usage are harvested from the scripts themselves.
...
| `batch-adoption-core.mjs` | THE ADOPTION RECORD AND ITS AGREEMENT PROBE — step 4 of the "Ordered work" in docs/handover-architecture.md (work-order point 834, the front stage of 676; union M17/M18). | — |
| `batch-attempt-lease-core.mjs` | THE ATTEMPT LEASE — step 4 of the "Ordered work" in docs/handover-architecture.md (work-order point 893, the front stage of 676; union M39/M40). | — |
...
| `batch-baseline-core.mjs` | Pure reconstruction of today's-path baseline from measured repository history. | — |
| `batch-baseline.mjs` | Record one immutable UTC baseline day from today's-path measured history. | usage: node scripts/batch-baseline.mjs --day YYYY-MM-DD --output <report.json> [--repo <dir>] [--ref main] |
| `batch-board-core.mjs` | READ-ONLY PROGRESS-BOARD PROJECTION — ordered-work step 10. | — |
...
| `board-heartbeat-core.mjs` | Pure core of the IN-TURN BOARD HEARTBEAT (point 848). | — |
| `board-heartbeat.mjs` | The I/O half of the IN-TURN BOARD HEARTBEAT (point 848). | — |
| `board-liveness-core.mjs` | THE BOARD'S OWN LIVENESS AND PROGRESS LINES — pure core. | — |
| `board-liveness.mjs` | THE READER SIDE OF THE BOARD'S LIVENESS AND PROGRESS LINES. | — |
...
| `decision-log-core.mjs` | MEASURED EXPIRY FOR AUTOMATIC DECISIONS (point 936). | — |
...
| `detached-agent.mjs` | THE COMMON WORKER CONTRACT — step 3 of the "Ordered work" in docs/handover-architecture.md (work-order point 834, the front stage of 676; union M5). | — |
| `detached-escape-drill.mjs` | WHY A CHILD DIES WITH THE SESSION THAT SPAWNED IT — measured, not argued. | — |
...
| `git-tracked.mjs` | Is a path a TRACKED, CLEAN repository artefact? | — |
| `guard-compatibility-core.mjs` | NO TWO GUARDS MAY JOINTLY DEMAND AND FORBID ONE ACTION (point 1048, union entry U18) — the pure half. | — |
...
| `launcher-stall-core.mjs` | Pure decision core of the launcher's OWN stall alarm (point 859). | — |
...
| `mechanism-review-merge-core.mjs` | SAFE THREE-WAY MERGE FOR THE APPEND-ONLY REVIEW LEDGER. | — |
| `mechanism-review-merge.mjs` | Git merge driver for .claude/mechanism-reviews.jsonl. | — |
...
| `module-link-check.mjs` | DO A MODULE'S NAMED IMPORTS ACTUALLY EXIST? | usage: node scripts/module-link-check.mjs <file> |
| `module-link-check.unresolvable-fixture.mjs` | FIXTURE, never loaded by anything that runs: the target does not exist at all. | — |
