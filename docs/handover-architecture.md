...
- **The spawning call NEVER awaits the daemon's WORK, but it does await its READINESS.** It
  ...
  call generated, its pid and pid start time probe live, and its state reads `running` — and a
  missing one of them keeps the caller waiting until the timeout, which fails the start rather
  than reporting one. The nonce is generated by the spawning call and passed in `argv`; a
  readiness record carrying any other nonce is a previous daemon's and does not satisfy the wait.
  Without it a stale record from a dead launch would end the wait immediately.
...
A crash between the two writes of STOP therefore leaves `stopping` behind, never `running`. START
is not its mirror image, because it has THREE durable writes rather than two — the record as
`starting`, the record as `running`, and the copy the lock owner installs afterwards — so it has
TWO crash windows. A crash in the first leaves `starting`, which is not adoptable and which
`start` never reports ready: the lock owner waits for `running` or reconciles the record as cold
once the process is gone. A crash in the second leaves `running` with the copy not yet installed,
which is the ordinary unadopted row — the lock owner probes the record and writes the copy.
...
writes leaves `stopping` and the lock owner does not adopt it; a crash in START's FIRST window
leaves `starting` with no copy, which the lock owner does not adopt and which `start` does not
report ready; and a crash in START's SECOND window — after the record reads `running`, before the
copy is installed — leaves the ordinary unadopted row, which the lock owner resolves by probing
the record and writing the copy. Covering only the first START window would pass over a design
that loses the daemon in the second.
