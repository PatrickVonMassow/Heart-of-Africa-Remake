...
| **Urlaubsfestigkeit** | N | Unattended operation for a fortnight — recovery from a failure at ANY moment, quota waiting, the boot path, the readiness check and the chaos drill that proves it | 442, 443, 444, 445, 446, 447, 448, 449, 450, 533, 562, 612, 504, 515, 517, 528, 616, 617, 658, 663, 716, 719, 760, 859, 861 (861 is the user's 23.08.2026 no-standstill order made standing: every state that parks the batch on a human gets a self-recovery lane or an own-judgment default with a retroactive-veto card, inventoried in docs/batch-autonomy.md — it reads the pause/gate/mandate paths this bundle owns and is worked after 859 and 860 supply its first two lanes; 859 is the standstill of 23.08.2026 that could not announce itself — a sick container hung every launcher tick for 3.5 h and every alert path needed a child process the container could not spawn; it edits the launcher daemon that 612 and 760 also own, so it is worked with them rather than beside them, and it is ranked FIRST by user order of 23.08.2026) |
