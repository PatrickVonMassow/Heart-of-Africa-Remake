...
| 3.42 | Erlaubt ist nicht genommen | 3 | LÜCKE: Die Erlaubnis existiert (`scripts/batch-progress-guard.mjs`, `scripts/batch-boundary.mjs`), der Vollzug nicht — eine erlaubte, aber nicht genommene Grenze schweigt heute, statt zu blockieren, und die Sperre bleibt beim Halt in der Hand des Eigentümers. Arbeitsauftrag 388 baut die fehlende Hälfte; bis dahin ist die Lehre erfasst und ausdrücklich NICHT durchgesetzt. |
| 3.43 | Ein Mechanismus, den ein Namenswechsel abschaltet | 3 | LÜCKE: Die Zustaendigkeit jedes besitzgebundenen Waechters haengt an der Sitzungs-ID (`scripts/batch-singleton-core.mjs`), und eine Kontext-Kompaktierung vergibt eine neue — die Sitzung haelt sich dann selbst fuer fremd und jeder Waechter tritt lautlos zurueck, auch der, der die Uebergabe eintraegt. Arbeitsauftrag 388 bindet die Identitaet an den Prozess (pid + Startzeit stehen bereits in der Sperre) und isoliert die Testpfade; bis dahin erfasst und ausdruecklich NICHT durchgesetzt. |
