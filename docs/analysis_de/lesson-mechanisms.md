...
| 3.56 | Die Marke, die ein Seitenstrich zurücknahm | 1 | Erfasst als Arbeitsauftrag 426, der den BESTEHENDEN Durchsetzer erweitert statt einen neuen danebenzustellen: `scripts/batch-boundary-core.mjs` (`isClosingSetCommand`) lässt einen reinen Pager am Ende einer Abschluss-Zeile stehen — ein `head`/`tail` ändert die Welt nicht —, und jede Rücknahme einer genommenen Grenze wird mit dem auslösenden Aufruf in `.claude/boundary.log` protokolliert, wo die Grenzereignisse ohnehin stehen. Bis der Punkt gebaut ist, steht der Fallstrick in `docs/batch-autonomy.md` unter der Grenz-Beschreibung. |
...
| 3.61 | Zwei Stunden erkannt, zwei Stunden nur notiert | 1 | Ein BESTEHENDER Durchsetzer wird erweitert, nicht ergänzt: `scripts/batch-autostart.mjs` stellt „WEDGED owner" bereits fest — ihm fehlt die Folge. Erfasst als Arbeitsauftrag 433 (Bündel I): dieselbe Stelle gibt den Lock frei und startet den Nachfolger, statt die Feststellung zu wiederholen; die Schwelle sinkt von vier Stunden auf ein Maß, das eine unbeaufsichtigte Nacht noch retten kann; und eine mehrfach gleichlautende Feststellung eskaliert, statt sich zu wiederholen. Kein neuer Wächter — der vorhandene bekommt Handlungsvollmacht. |
...
| 3.58 | Ein Agent in Arbeit, zwei Plätze leer | 1 | Erfasst als Arbeitsauftrag 427, der die BESTEHENDE Wartemeldung erweitert: `scripts/batch-in-flight.mjs` zählt die laufenden Agenten und die offenen, unabhängigen Punkte und verlangt bei freien Poolplätzen eine Begründung; `scripts/batch-progress-guard.mjs` lässt den Zug enden, sobald sie vorliegt — kein neuer Wächter, sondern eine Untergrenze in dem, der die Wartezeit ohnehin beurteilt. |
