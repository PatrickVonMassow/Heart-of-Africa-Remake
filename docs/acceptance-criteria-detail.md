...
per panel, not a circle per post); each animal collides with the others. A scattered stone
counts as an obstacle only above the step height `balance.placeStepOverTop` (0.30 m,
calibratable): below it the stone raises the walking surface instead — it holds no collider
and player and inhabitants ride over it, as they do over an excavation's spoil. No inhabitant is
