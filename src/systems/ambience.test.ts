...
describe('the loom reed has a placed, dry beat', () => {
  it('uses speech falloff and panning, including the rear half of the village', () => {
    ...
    for (const d of [1, 3, 5, balance.communication.talk.reach]) {
      expect(loomBeatPlan(d, 0, 1).peak / near.peak).toBeCloseTo(hearingGain(d))
    }
    expect(loomBeatPlan(balance.communication.talk.reach + 1, 0, 1).peak).toBe(0)
