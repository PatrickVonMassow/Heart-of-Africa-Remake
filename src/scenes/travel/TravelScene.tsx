...
import { travelBlockedAt, useGame } from '../../state/store'
import {
  useUi,
  effectiveShadows,
  ...
  useFrame((_, rawDt) => {
    ...
    if (!useUi.getState().dialog) {
      ...
      if (obstacles.length > 0) {
        ...
        // A PUSH out of an animal or a tree must not carry him from open ground
        // into blocked water (point 1212: an antelope wading onto him at the
        // delta shore pushed him into the closed Mediterranean). He holds his
        // last spot instead. His own step is left alone (moveTravel judged it),
        // and one already stranded keeps the push, so no progress out is undone.
        const pushed = nx !== p.x || nz !== p.z
        if (pushed && !travelBlockedAt(beforeX, beforeZ, s.seed) && travelBlockedAt(nx, nz, s.seed)) [nx, nz] = [beforeX, beforeZ]
