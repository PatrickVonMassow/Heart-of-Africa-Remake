...
15. **Lively, densely built settlements.** `design.md` §2.6 (dense
    ...
    hard grey horizon line with a thin blue-grey water band below it —
    the gate (`panoramaCaptureReady`) is pure-tested in
    `src/scenes/travel/panoramaMath.test.ts` and the leave-capture's
    band is live-checked to bake the surrounding terrain (bottom-quarter
